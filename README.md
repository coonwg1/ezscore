...
`ezscore-f` provides ready-to-use sleep staging models for two-channel forehead EEG. The models support offline and real-time scoring and include an artifact class designed to isolate the signal contamination commonly encountered with wearable forehead devices.

- 📦 Ready-to-use model checkpoints (`ez6`, `ez6rt`, and `ez6moe`)
- 📈 Offline and real-time scoring
- 🧪 Artifact-aware six-class output
- 🧠 ZMax, DCM, and CGX PatchEEG data loaders

> [!IMPORTANT]
> **CGX PatchEEG is now supported.** EzScore can load both raw `.cgx` recordings and PatchEEG EDF exports, automatically apply the required channel re-referencing, and return model-ready left/right forehead EEG.
...
EzScore models were trained on ZMax data and validated on forehead EEG datasets described in the accompanying paper. See the citation below for model-development and validation details.

## What's new

### CGX PatchEEG compatibility — July 2026

The new `load_patcheeg()` loader supports:
...
The default summary-plot palette has also been refreshed. Hypnograms and hypnodensities now use a magma-derived stage palette, while spectrograms use the `magma` colormap. The original EzScore colors remain available with `original_colors=True`.
...
All loaders return an MNE `Raw` object with EEG channels named `eegl` and `eegr`, sampled at 64 Hz.
...
Once a model is loaded, score and plot the recording with the standard utilities:
